...
def get_conv_net(X):
    ...
    tf.add_to_collection('embed', pool1)
    fc1 = tf.layers.dense(embed, 300, activation=tf.nn.relu, )
    #fc1 = tf.layers.batch_normalization(fc1, training=is_training)
    #if do not want to use dropout, comment the line below and uncomment the line above
    fc1 = tf.layers.dropout(inputs=fc1, rate=0.4)
...
with tf.Session() as sess:
    ...
    #plot the filter of conv layer
    conv_weights = params[0].eval()
    plot_conv_weights(conv_weights, dir_prefix, params[0].name)
    ...
    conv_out = sess.run([tf.get_collection('embed')], feed_dict={X: img, is_training: False})
    conv_out = np.array(conv_out)[0,0]
    plot_conv_output(conv_out, "./output", 'embed')
    plt.show()
    sess.close()    
...
# for idx, v in enumerate(params):
    # plot_conv_weights(v, dir_prefix, v.name)
